...
check_gateway_handshake_runtime_hotfix() {
  ...
  if [[ ${#method_files[@]} -gt 0 ]]; then
    if rg -q 'const DEFAULT_PREAUTH_HANDSHAKE_TIMEOUT_MS = 15e3;' "${method_files[@]}"       && rg -q 'OPENCLAW_HANDSHAKE_TIMEOUT_MS' "${method_files[@]}"       && rg -q 'OPENCLAW_TEST_HANDSHAKE_TIMEOUT_MS' "${method_files[@]}"; then
      return 0
    fi
  fi
  ...
  if rg -q 'const DEFAULT_HANDSHAKE_TIMEOUT_MS = 15e3;' "${gateway_files[@]}"     && (rg -q 'OPENCLAW_GATEWAY_HANDSHAKE_TIMEOUT_MS' "${gateway_files[@]}" || rg -q 'OPENCLAW_HANDSHAKE_TIMEOUT_MS' "${gateway_files[@]}")     && rg -q 'OPENCLAW_TEST_HANDSHAKE_TIMEOUT_MS' "${gateway_files[@]}"; then
    return 0
  fi
  rg -q 'option\("--timeout <ms>", "Timeout in ms", "15000"\)' "${gateway_files[@]}" \
    && rg -q 'timeoutMs: Number\(opts\.timeout \?\? 15e3\)' "${gateway_files[@]}"
}
...
apply_gateway_handshake_runtime_hotfix() {
  ...
  mapfile -t method_files < <(ls -1 "${DIST_DIR}"/method-scopes-*.js 2>/dev/null || true)
  if [[ ${#method_files[@]} -gt 0 ]]; then
    if rg -q 'const DEFAULT_PREAUTH_HANDSHAKE_TIMEOUT_MS = 1e4;' "${method_files[@]}"; then
