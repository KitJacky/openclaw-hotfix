...
## Current Baseline
- OpenClaw version (at last verification): `2026.4.11 (769908e)`
...
- Newer build note (2026.4.11):
  - the handshake-specific constants may be absent, but `gateway-cli-*.js` still defaults CLI timeout to `10000` / `1e4`
  - treat `option("--timeout <ms>", "Timeout in ms", "15000")` plus `timeoutMs: Number(opts.timeout ?? 15e3)` as the accepted patched state
