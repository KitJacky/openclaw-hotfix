...
## Current Baseline
- OpenClaw version (at last verification): `2026.5.22 (a374c3a)`
...
- Host verification snapshot (after warm-up):
  ...
  - `openclaw-post-update-hotfix.sh --check` -> all hotfix checks passed
  - `openclaw gateway health --timeout 120000 --json` -> `ok: true`
  - `openclaw cron status --json` -> `jobs: 22`
  - `openclaw cron run 8888 --timeout 120000` -> enqueued OK
  - `openclaw security audit` -> `0 critical · 1 warn · 6 info` (unpinned Brave spec; other closed-system downgrade rules still produce info-level items as documented below)

### 2026.5.7
- Upgrade path from this host's previous stable baseline `2026.4.26` to `2026.5.7` was successful after re-applying host hotfixes and installing the external Brave plugin required by the new web-search provider layout.
- `2026.5.7` directly improves several host-relevant paths: Telegram polling watchdog correctness, owner enforcement for native commands, stale context invalidation after `/new` and reset flows, cached skills snapshot reset on `/new`/`sessions.reset`, and Tavily credential resolution from active runtime config.
- Post-upgrade host action required: install `@openclaw/brave-plugin`, confirm `plugins.entries.brave.enabled=true`, restart gateway, and re-run `cron status` until the `tools.web.search.provider` warning disappears.
- Host verification after upgrade:
  - `openclaw --version` -> `OpenClaw 2026.5.7 (eeef486)`
  - `openclaw-post-update-hotfix.sh --check` -> all hotfix checks passed
  - `openclaw gateway health --timeout 120000 --json` -> `ok: true` with warm response around `1.3-2.8s`
  - `openclaw cron status --json` -> clean, `jobs: 22`, no Brave warning after plugin install
  - `openclaw security audit` -> `0 critical · 0 warn · 6 info`

### 2026.4.26
- `openclaw update` now installs into a validated temporary prefix before switching live. This reduces mixed-file and stale-chunk upgrade failures, but package hotfixes still need to be re-applied because bundle filenames can change.
- Foreground gateway startup no longer uses the old CLI self-respawn path. This is good for low-memory Linux hosts and reduces one class of startup hangs.
- Broken-pipe / `EPIPE` stream shutdown handling is more forgiving upstream, so normal response-complete pipe closures are less likely to take down gateway-side flows.
- On this host, the first `gateway health` after restart can be much slower than `2026.4.14/15`. Observed cold-start healthy response window on `2026.4.26`: up to about `81s`.
- Once warm, the same host can answer `gateway health` much faster again. Treat this as a warm/cold-path difference, not a random failure.

### 2026.4.15
- Anthropic defaults move to Claude Opus 4.7 aliases/defaults. Local model config is unchanged on this host.
- Gemini TTS support is added through the bundled `google` plugin. This increases plugin/dependency surface but does not require local config changes.
- Startup/skills/memory prompt budgets are trimmed upstream. This is compatible with this host's `thinkingDefault="low"` and `llm.idleTimeoutSeconds=900` policy.
- npm upgrades now prune stale packaged `dist` chunks. This is good for stale import failures, but it means package hotfixes must be rechecked after every upgrade because patched chunk filenames will change.
- Gateway/tool name collision hardening is upstream security work. No local bypass is needed.
...
Patch target:
- `/usr/lib/node_modules/openclaw/node_modules/@earendil-works/pi-ai/dist/providers/openai-completions.js` (current, e.g. `2026.5.22+`)
- Legacy: `@mariozechner/pi-ai/...` (older installs — host hotfix script checks both)

Expected patched logic:
- `params.stream_options = { include_usage: true };`
- Must NOT contain gate: `if (compat.supportsUsageInStreaming !== false) { ... }`

### 3) cron.run tool timeout guard (10-15 minutes)
### 3A) LLM idle timeout guard (15 minutes)
- Config:
  - legacy path: `agents.defaults.llm.idleTimeoutSeconds = 900`
  - newer provider path: `models.providers.<providerId>.timeoutSeconds >= 900`
- Why: self-hosted local models can exceed 300s during long-context or slow-token runs, especially in cron jobs.
- Policy: prefer `900` for autonomous jobs; avoid `0` unless you intentionally want no idle cutoff.
- `2026.4.26` doctor note:
  - `agents.defaults.llm` is now treated as a legacy config shape upstream.
  - Future migrations may move this timeout to `models.providers.<id>.timeoutSeconds`.
  - The hotfix checker must accept either location as valid.

### 3B) Default thinking level guard
- Config: `agents.defaults.thinkingDefault = "low"`
- Why: `medium` increases the chance of long self-talk, repeated partial summaries, and runaway research loops on local models.
- Policy: use `low` as the default for autonomous cron work. Raise thinking per task only when a specific workflow proves it needs more deliberate reasoning.
...
Expected patched logic:
- Newer builds:
  - upstream `2026.4.14` already sets `cron run` default timeout to `600000` (10m).
  - local hotfix may still raise this to `900000` (15m) for long-running autonomous jobs.
- Legacy builds:
...
### 6A) Disable unconfigured MiniMax web-search fallback on this host
Reason:
- `2026.4.26` can expose bundled provider/plugin candidates that are not truly usable on this host.
- In practice, `web_search` fallback attempts were surfacing `missing_minimax_api_key` tool results into autonomous sessions.
- Those tool results were recorded as normal tool output instead of a hard runtime failure, which can pollute local-model context and induce bad follow-up reasoning.
...
Host policy:
- This host uses Brave as primary `web_search` and Tavily as explicit backup.
- MiniMax must stay disabled unless real MiniMax credentials are intentionally provisioned.
- Do not expose provider/plugin surfaces to the LLM unless the corresponding credentials and routing policy are intentionally enabled.
...
Reason:
- On this host, Telegram `/status` can still respond while bare `/new` and `/reset` appear to do nothing.
- Those commands do not send an immediate ACK upstream; they continue into a full agent startup turn.
- With this workspace's startup files and default runtime startup-context prelude, the first-turn reset path can become too heavy for practical Telegram responsiveness and leaves `agent:main:telegram:slash:*` sessions stuck in `processing`.
...
Host policy:
- Keep normal Telegram chat replies unchanged.
- Only remove the extra runtime-loaded daily-memory prelude for bare `/new` and `/reset`.
- If slash sessions are already wedged, delete stale `agent:*:telegram:slash:<ownerId>` entries from the session stores and restart `openclaw-gateway.service`.

Verification:
- `openclaw doctor` may still show Telegram pairing warnings; those warnings do not mean `/new` or `/reset` are unsupported.
- `openclaw gateway` logs should stop repeatedly reporting stuck slash sessions for the owner after a clean retry.
- `/status` should continue to work, and `/new` / `/reset` should create a fresh slash session instead of reusing a dead one.
...
- Newer build note (2026.4.14 / 2026.4.15):
  - handshake constants live in `client-*.js`
  - patch target:
    - `const DEFAULT_PREAUTH_HANDSHAKE_TIMEOUT_MS = 15e3`
    - env precedence should accept `OPENCLAW_GATEWAY_HANDSHAKE_TIMEOUT_MS` before `OPENCLAW_HANDSHAKE_TIMEOUT_MS`
  - `gateway-cli-*.js` still needs CLI timeout default raised from `10000` to `15000`
...
Host note for `2026.4.26`:
- the first `openclaw gateway health` after restart may take as long as about `81s` on this host
- prefer `openclaw gateway health --timeout 90000 --json` for post-upgrade verification
- once warm, the same host can answer health checks much faster; do not treat the cold-start path as a hard failure unless it exceeds the longer timeout

Host note for `2026.4.14` / `2026.4.15`:
- `openclaw gateway call health --timeout 20000 --json` can still time out on this host even when the gateway is healthy.
- Observed healthy response window: about `42-44s`.
- Prefer `openclaw gateway health --timeout 60000 --json` for post-upgrade verification.
- `openclaw gateway status --json` can report `rpc.error="timeout"` while `health.healthy=true` because its internal RPC probe uses a shorter timeout.
...
### 10) Telegram bundled setup-entry compatibility (2026.4.7 / 2026.4.9)
Reason:
...
- `2026.4.9` changed the bundle layout and now uses valid split specifiers:
  - `./setup-plugin-api.js`
  - `./secret-contract-api.js`
- The hotfix checker must therefore accept either the repaired legacy layout or the newer upstream-valid layout.

Patch target:
- `/usr/lib/node_modules/openclaw/dist/extensions/telegram/setup-entry.js`

Accepted valid states:
- legacy repaired layout:
  - `plugin.specifier = "./api.js"`
  - `plugin.exportName = "telegramSetupPlugin"`
- upstream-valid `2026.4.9` layout:
...
- `/usr/lib/node_modules/openclaw/node_modules/@earendil-works/pi-ai/dist/providers/openai-completions.js` (or legacy `@mariozechner/pi-ai/...`)
