...
# pi-ai npm scope migrated (e.g. 2026.5.22); keep legacy path for older installs.
if [[ -f "${OPENCLAW_ROOT}/node_modules/@earendil-works/pi-ai/dist/providers/openai-completions.js" ]]; then
  PROVIDER_FILE="${OPENCLAW_ROOT}/node_modules/@earendil-works/pi-ai/dist/providers/openai-completions.js"
elif [[ -f "${OPENCLAW_ROOT}/node_modules/@mariozechner/pi-ai/dist/providers/openai-completions.js" ]]; then
  PROVIDER_FILE="${OPENCLAW_ROOT}/node_modules/@mariozechner/pi-ai/dist/providers/openai-completions.js"
else
  PROVIDER_FILE="$(find "${OPENCLAW_ROOT}/node_modules" -path '*/pi-ai/dist/providers/openai-completions.js' -type f -print -quit)"
fi
...
find_web_search_runtime_file() {
  rg -l 'async function runWebSearch\(params\)' "${DIST_DIR}"/runtime-*.js "${DIST_DIR}"/*.js 2>/dev/null | head -n 1
}
...
find_client_file() {
  rg -l 'DEFAULT_PREAUTH_HANDSHAKE_TIMEOUT_MS|OPENCLAW_GATEWAY_HANDSHAKE_TIMEOUT_MS|OPENCLAW_HANDSHAKE_TIMEOUT_MS' \
    "${DIST_DIR}"/handshake-timeouts-*.js "${DIST_DIR}"/client-*.js "${DIST_DIR}"/active-client-*.js 2>/dev/null | head -n 1
}
...
ok = (
    isinstance(legacy_value, int) and legacy_value >= 900
) or (
    isinstance(provider_timeout, int) and provider_timeout >= 900
)
...
check_gateway_rpc_config_hotfix() {
  ...
  if retry_quiet 3 "$OPENCLAW_BIN" cron status --json && retry_quiet 3 "$OPENCLAW_BIN" gateway call health --timeout 20000 --json; then
    return 0
  fi
...
check_gateway_handshake_runtime_hotfix() {
  ...
  rg -q 'const DEFAULT_PREAUTH_HANDSHAKE_TIMEOUT_MS = 15e3;' "$client_file" \
    ...
    && { [[ -z "$gateway_file" ]] || ! rg -q 'option\("--timeout <ms>", "Timeout in ms", "10000"\)' "$gateway_file"; }
}
...
if not isinstance(current, int) or current < 900:
    ...
    provider_timeout = local_provider.get("timeoutSeconds")
    if not isinstance(provider_timeout, int) or provider_timeout < 900:
        local_provider["timeoutSeconds"] = 900
...
minimax = entries.setdefault("minimax", {})
...
main() {
  ...
  case "$MODE" in
    ...
    --apply)
      ...
      if ! check_minimax_disabled_hotfix; then
